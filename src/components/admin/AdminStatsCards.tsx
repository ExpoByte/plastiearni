...
export const AdminStatsCards = ({ stats }: AdminStatsCardsProps) => {
  const statCards = [
    ...
    },
    {
      ...
      value: stats?.total_points_balance?.toLocaleString() ?? "0",
      color: "bg-yellow-500/10 text-yellow-500",
    },
    {
      icon: Scale,
      label: "Plastic Collected",
      value: `${Number(stats?.total_kg_collected ?? 0).toFixed(1)} kg`,
    ...
    },
    {
      icon: Banknote,
      label: "Redemptions",
      value: `${stats?.successful_redemptions ?? 0}/${stats?.total_redemptions ?? 0}`,
